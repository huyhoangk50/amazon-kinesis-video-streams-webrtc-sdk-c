...
STATUS createTlsSession(PTlsSessionCallbacks pCallbacks, PTlsSession *ppTlsSession)
{
    ...
    CHK_STATUS(createIOBuffer(MAX_UDP_PACKET_SIZE, &pTlsSession->pReadBuffer));
...
STATUS tlsSessionProcessPacket(PTlsSession pTlsSession, PBYTE pData, UINT32 bufferLen, PUINT32 pDataLen)
{
    ...
    while (pReadBuffer->off < pReadBuffer->len) {
        sslRet = mbedtls_ssl_read(&pTlsSession->sslCtx, pData + readBytes, pReadBuffer->len - pReadBuffer->off);
        if (sslRet > 0) {
            readBytes += sslRet;
